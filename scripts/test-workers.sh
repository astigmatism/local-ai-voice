...
python3 -m compileall -q workers/stt-fast-whisper/app workers/tts-chatterbox/app workers/tts-kokoro/app
...
if [ -x "workers/tts-kokoro/.venv/bin/pytest" ]; then
  (cd workers/tts-kokoro && .venv/bin/pytest)
fi
